...
(* parses entire program as one S-expresion *)
let parse_string_full (s: string) : Expr.prog = 
    match Sexp.of_string ("(" ^ s ^ ")") with 
    | List(sexps) -> Parser.parse_program sexps
    | Atom(_) -> failwith "Impossible"

(* takes a file and produces a list of S-expressions out of the file *)
let parse_file_full input_file =
  let sexps = Sexp.input_sexps input_file in
...
let rec waitpids (pid1: int) (pid2: int) : int * process_status =
  let pid, status = Unix.waitpid ([]) (-1) in
    if pid = pid1 || pid = pid2 then (pid, status)
    else waitpids pid1 pid2
;;
...
(* tries to parse a whole program *)
...
let test_err program_str outfile errmsg _ =
  ...
  | Left(e) -> 
   ...
   | Right(prog) -> 
  let result = run prog full_outfile in
  assert_equal
    (Left(errmsg))
    result
    ~printer:either_printer
